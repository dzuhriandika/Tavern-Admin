...
export default class Component extends React.Component {
  ...
  editMenu(data) {
    this.props.navigation.navigate('EditMenu', { data });
  }
  ...
  renderList() {
      ...
      const list = this.state.data.map(data => (
        <View style={styles.wrapperCard} key={data.menuId}>
          ...
          <View style={styles.wrapperAction}>
            <TouchableOpacity style={styles.editAction} onPress={() => this.editMenu(data)}>
              <Pen />
            </TouchableOpacity>
    ...
    return <Text style={{ textAlign: 'center', marginTop: 30 }}>No Data</Text>;
  }
